...
class CustomerDbHelper {
  ...
  Future<List> getQuestionData() async {
    var response = await http.get(Uri.parse("http://54.180.102.153:18080/api/monthlyPick"));
    return json.decode(response.body);
  }

  Future<CustomerQuestion> getUserData() async {
    final response = await http.get(Uri.parse("http://54.180.102.153:18080/api/monthlyPick/{id}"));
  ...
  Future<CustomerQuestion> updateQuestion(String id, String title, String contents) async {
    final response = await http.put(
      Uri.parse('http://54.180.102.153:18080/api/monthlyPick/' + id),
      headers: <String, String>{
        'Content-Type': 'application/json; charset=UTF-8',
      },
      body: jsonEncode(<String, String>{
  ...
  Future<http.Response> deleteQuestion(String id) async {
    var url = 'http://54.180.102.153:18080/api/monthlyPick/'+ id; // id 부분은 따로 빼줘야 함, 여태까지 string 으로 인식하고  있어서 처리가 안됨
